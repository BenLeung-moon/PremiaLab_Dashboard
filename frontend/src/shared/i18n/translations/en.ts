export default {
  chat: {
    title: "I'm DeepSeek, nice to meet you!",
    welcome: 'I can help you analyze portfolios, provide market insights, and answer investment-related questions.',
    inputPlaceholder: 'Send a message to DeepSeek',
    apiKeyRequired: 'Please set up your API key to continue',
    apiCallFailed: 'Request failed, please try again later',
    portfolioSent: 'Portfolio has been successfully sent to the dashboard!',
    backendError: 'Failed to send, please try again later',
    disclaimer: 'Content is AI-generated, please verify carefully',
    examples: {
      tech: 'Analyze the performance of a tech stock portfolio',
      createPortfolio: 'Help me create a portfolio with Apple, Microsoft, Google, and Amazon',
      techAllocation: 'Recommend the best allocation ratio for tech stocks',
      lowRisk: 'Create a low-risk investment portfolio'
    }
  },
  settings: {
    title: 'API Settings',
    description: 'Please enter your Perplexity API key',
    apiLinkText: 'Get API Key',
    placeholder: 'Enter API Key',
    save: 'Save',
    cancel: 'Cancel'
  },
  navigation: {
    toDashboard: 'View Dashboard'
  },
  portfolio: {
    sendToDashboard: 'Send to Dashboard',
    analysis: {
      title: 'Portfolio Analysis',
      overview: 'Portfolio Overview',
      details: 'Detailed Analysis',
      suggestions: 'Suggestions'
    }
  }
}; 